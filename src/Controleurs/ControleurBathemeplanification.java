...
import View.PlanificationBatheme;
...
public class ControleurBathemeplanification implements ActionListener
{

    private PlanificationBatheme batheme;
    private String filepathplanning;
    private ClassSingleton singleton;
    private LocalDate DateCourante;
    public ControleurBathemeplanification(PlanificationBatheme planificationBatheme)
    {
        batheme = planificationBatheme;
    ...
    public void EcritureDesJours(int jourdumoi, int nbjour, int moi, int annee)
    {
        ...
        batheme.getListJourDuMoi().setModel(list);
    }
    ...
    public int CalculeNbJour(String lemoi)
    {
        ...
        switch (lemoi)
        {
            ...
            case "Novembre":
                ...
                break;
            case "Février":
                String strinanntmp = batheme.getLabelAnnee().getText();
    ...
    {
        if(e.getSource() == batheme.getbuttonPrevius())
        {
            int moi = DateCourante.getMonthValue();
            String moistr = batheme.getLabelMoi().getText();
            int moiaffiche = ConvertionMoiEnInt(moistr);
            int anneeaffiche = Integer.parseInt(batheme.getLabelAnnee().getText());
            ...
            {
                if(ConvertionMoiEnInt(batheme.getLabelMoi().getText()) == 1) //retour a l'année précédante
                {
                    String tmp = batheme.getLabelAnnee().getText();
                    int annee = Integer.parseInt(tmp);
                    annee--;

                    batheme.getLabelAnnee().setText("" + annee);
                    batheme.getLabelMoi().setText("Décembre");
                ...
                {
                    //le moi est trouvé
                    int nummoi = ConvertionMoiEnInt(batheme.getLabelMoi().getText());
                    nummoi--;
                    batheme.getLabelMoi().setText(ConvertionMoiEnFR(nummoi));

                    int jourdumoi = 1;
                    if(nummoi == DateCourante.getMonthValue() && Integer.parseInt(batheme.getLabelAnnee().getText()) == DateCourante.getYear())
                    {
                        jourdumoi = DateCourante.getDayOfMonth();
                    }

                    int nbmaxjour = CalculeNbJour(batheme.getLabelMoi().getText());

                    EcritureDesJours(jourdumoi, nbmaxjour, ConvertionMoiEnInt(batheme.getLabelMoi().getText()), Integer.parseInt(batheme.getLabelAnnee().getText()));
                }
            }
        }
        else if(e.getSource() == batheme.getButtonNext())
        {

            if(ConvertionMoiEnInt(batheme.getLabelMoi().getText()) == 12) //année suivante
            {
                String tmp = batheme.getLabelAnnee().getText();
                int annee = Integer.parseInt(tmp);
                annee++;
                ...
                batheme.getLabelAnnee().setText("" + annee);
                batheme.getLabelMoi().setText("Janvier");
        ...
        else if(e.getSource() == batheme.getButtonTerminer())
        {
            String valeuractuelle = batheme.getListJourDuMoi().getSelectedValue();

            JOptionPane jop = new JOptionPane();
            if(valeuractuelle != null)
            {
                if(!valeuractuelle.contains("bathème")) //ok on peu prévoir
                {
                    ...
                    LocalDate date = LocalDate.of(Integer.parseInt(batheme.getLabelAnnee().getText()), ConvertionMoiEnInt(batheme.getLabelMoi().getText()), jour);

                    //recherche de l'élément s'il existe déjà
                    int index = 0;
                    boolean SuppressionDsFichier = false;
                    ClassJour lejour = new ClassJour(date, true, false, false);
                    ...
                    try (BufferedWriter writer = new BufferedWriter(new FileWriter(filepathplanning, true))) { //creation du fichier ici
                        ...
                        jop.showMessageDialog(null, "Bathème enregistré", "Information", jop.INFORMATION_MESSAGE);
                        jop.setVisible(true);
                        batheme.dispose();
                ...
                {
                    jop.showMessageDialog(null, "Veuillé sélectionner une autre date, un bathème est déjà prévu", "Erreur", jop.ERROR_MESSAGE);
                    jop.setVisible(true);
                }
            }
            else
